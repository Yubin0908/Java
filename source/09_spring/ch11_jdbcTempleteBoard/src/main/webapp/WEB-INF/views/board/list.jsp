...
<body>
	...
	<c:if test="${deleteMsg eq SUCCESS }">
		<script>alert('${param.bid}번 글삭제 성공');</script>
	</c:if>
	<c:if test="${deleteMsg eq FAIL }">
	...
	<div class="paging">
		<c:if test="${startPage > blocksize }">
			<a href="${conPath }/board/list.do?page=${startPage-1}">이전</a>
		</c:if>
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			...
			<c:if test="${i ne page }">
			[ <a href="${conPath }/board/list.do?page=${i}">${i }</a> ]
			</c:if>
		</c:forEach>
		<c:if test="${endPage < pageCnt }">
			[ <a href="${conPath }/board/list.do?page=${endPage+1}">다음</a> ]
		</c:if>
	</div>
</body>
</html>