-- SubQuery : 메인 QUERY(SQL문) DKSDP 내포
-- 1. SUBQUERY 개념
  -- 서브쿼리가 필요한 예제. 급여를 제일 많이 받는 사람의 모든 정보
SELECT MAX(SAL)
FROM EMP; -- 서브쿼리
...
SELECT E.*, DNAME, LOC
FROM EMP E, DEPT D 
WHERE E.DEPTNO=D.DEPTNO AND LOC= (SELECT LOC FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO AND ENAME='SCOTT') AND ENAME <> 'SCOTT';
...
SELECT ENAME, HIREDATE
  ...
  -- EX. 최초입사직원이름과 최초입사일, 최근입사직원이름과 최근입사일
...
SELECT SUM(SAL) 
  ...
  -- EX. SCOTT과 동일한 JOB을 가진 사원의 모든 필드(정보)
...
SELECT * 
  ...
  -- EX. DALLAS에 근무하는 사원의 이름과 부서번호 (서브쿼리// JOIN 사용)
...
SELECT ENAME, D.DEPTNO
  FROM EMP E, DEPT D
  WHERE E.DEPTNO=D.DEPTNO AND LOC='DALLAS'; -- JOIN
  
  -- EX. KING이 직속상사인 사원의 이름과 급여 (서브쿼리, SELF-JOIN)
...
SELECT W.ENAME, W.SAL
  FROM EMP W, EMP M
  WHERE W.MGR=M.EMPNO AND M.ENAME='KING'; -- JOIN
...
SELECT ENAME, SAL
  ...
  -- EX. 평균급여 이하로 받는 사원의 이름, 급여, 평균급여(소수점 1자리)를 출력
SELECT ENAME, SAL, ROUND((SELECT AVG(SAL) FROM EMP), 1) AVGSAL
  FROM EMP
  WHERE SAL <= (SELECT ROUND(AVG(SAL), 1) FROM EMP);

  -- EX. 평균급여 이하로 받는 사원의 이름, 급여, 평균급여와의 차이를 출력
SELECT ENAME, SAL, ROUND((SELECT AVG(SAL) FROM EMP)-SAL) LEFT
  FROM EMP
  WHERE SAL <= (SELECT ROUND(AVG(SAL), 1) FROM EMP);
...
--=========================================================================================================================--
  -- 서브쿼리의 종류 (2) 다중행 서브쿼리(서브쿼리의 실행결과가 2행이상) : IN, ALL, ANY
  -- EX. SCOTT이나 KING과 동일한 부서에서 근무하는 사원의 이름, 급여
SELECT DEPTNO FROM EMP WHERE ENAME IN('SCOTT', 'KING'); -- 다중행 서브쿼리
...
SELECT * 
  ...
  -- (2) ANY(=SOME) : 서브쿼리 결과가 하나이상 만족하면 참
  -- 30번 부서 직원 한명이라도 급여가 큰 직원의 모든 필드
...
SELECT *
  FROM EMP
  WHERE SAL > (SELECT MIN(SAL) FROM EMP WHERE DEPTNO=30); -- 단일행 이용방법


  -- (2) EXISTS : 서브쿼리 결과가 존재하면 참
...
SELECT EMPNO, ENAME, JOB, HIREDATE, SAL, GRADE
  FROM EMP, SALGRADE
  WHERE SAL BETWEEN LOSAL AND HISAL AND (GRADE, SAL) IN (SELECT GRADE, MAX(SAL) FROM SALGRADE, EMP WHERE SAL BETWEEN LOSAL AND HISAL AND SAL>=3000 GROUP BY GRADE);
...
SELECT TO_CHAR(HIREDATE, 'Q'), MAX(SAL)
  FROM EMP
  GROUP BY TO_CHAR(HIREDATE, 'Q');    -- 분기별 최고급여
...
SELECT ENAME, SAL, JOB
  FROM EMP
  WHERE SAL > ALL(SELECT SAL FROM EMP WHERE JOB='SALESMAN');      -- ALL 

SELECT ENAME, SAL, JOB
  FROM EMP
  WHERE SAL > (SELECT MAX(SAL) FROM EMP WHERE JOB='SALESMAN');    -- 단일행

-- 탄탄7. SALESMAN 일부 어떤 한 사원보다 급여를 많이 받는 사원들의 이름과 급여와 직급(담당 업무)를 출력하되 영업 사원도 출력(ANY)
SELECT ENAME, SAL, JOB
FROM EMP
WHERE SAL > ANY(SELECT SAL FROM EMP WHERE JOB='SALESMAN');      -- ALL 
  
SELECT ENAME, SAL, JOB
  FROM EMP
  WHERE SAL > (SELECT MIN(SAL) FROM EMP WHERE JOB='SALESMAN');    -- 단일행  
--=========================================================================================================================--
  -- 서브쿼리의 종류 (3) 다중열 서브쿼리(서브쿼리의 실행결과가 2행이상) : 
...
SELECT ENAME, SAL, HIREDATE, GRADE||'등급'
  FROM EMP E, DEPT D, SALGRADE
  WHERE E.DEPTNO=D.DEPTNO AND TO_CHAR(HIREDATE, 'RR/MM/DD') < (SELECT HIREDATE FROM EMP WHERE ENAME='SCOTT') AND SAL BETWEEN LOSAL AND HISAL;

  --6. 5번(SCOTT보다 먼저 입사한 사람의 이름, 급여, 입사일, 급여 등급)에 부서명 추가하고 급여가 큰 순 정렬
SELECT ENAME, SAL, HIREDATE, GRADE||'등급', DNAME
  FROM EMP E, DEPT D, SALGRADE
  WHERE E.DEPTNO=D.DEPTNO AND TO_CHAR(HIREDATE, 'RR/MM/DD') < (SELECT HIREDATE FROM EMP WHERE ENAME='SCOTT') AND SAL BETWEEN LOSAL AND HISAL
  ORDER BY SAL DESC;
...
SELECT EMPNO, ENAME, SAL
  FROM EMP E, DEPT D
  WHERE E.DEPTNO=D.DEPTNO AND (E.DEPTNO) = (SELECT D.DEPTNO FROM DEPT D, EMP E WHERE D.DEPTNO=E.DEPTNO AND E.ENAME='CLARK')
...
SELECT EMPNO, ENAME, SAL
  ...
  WHERE E.DEPTNO=D.DEPTNO AND SAL > (SELECT SAL FROM EMP WHERE EMPNO='7698')
                          AND DNAME = (SELECT D.DNAME FROM DEPT D, EMP E WHERE E.DEPTNO=D.DEPTNO AND ENAME='CLARK');
...
SELECT ENAME, JOB, SAL
  FROM EMP
  WHERE (SAL, SAL) IN (SELECT SAL, SAL FROM EMP WHERE ENAME='SCOTT' OR ENAME='WARD');
  
  -- 22. CHICAGO에서 근무하는 사원과 같은 업무를 하는 사원들의 이름,업무
SELECT ENAME, JOB
  FROM EMP 
  WHERE JOB IN (SELECT JOB FROM DEPT D, EMP E WHERE E.DEPTNO=D.DEPTNO AND LOC='CHICAGO') ;
...
SELECT DEPTNO, ENAME, SAL, (SELECT ROUND(AVG(SAL)) FROM EMP WHERE JOB=E.JOB) JOBAVG
  FROM EMP E
